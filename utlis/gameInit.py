...
class GameClass:
    ...
    def generateActionWorkMap(self,workMapTemp=[]):
        X = Int('X')
        X1 = Int('X1')
        X2 = Int('X2')
        Y = Int('Y')
        ...
        k = Int('k')
        l = Int('l')
        workMap=copy.deepcopy(workMapTemp)
        if self.Game['var_num'] == 1:
            for actoin in self.Game["actions"]:
                ...
                actoinParams=[]
                # 动作参数为1的情况
                actoinParamName1=actoin['action_parameter'][0]
                if(len(actoin['action_parameter'])==2):
                    actoinParam2=actoin['action_parameter'][1]
                print(workMapTemp,'workMapTemp')
                if(not workMapTemp):
                    while(True):
                        s = Solver()
                        ...
                        for params in actoinParams:
                            s.add(Or(actoinParamName1!=params))
                        if(s.check() == sat):
                            m = s.model()
                            print(m)
                            actoinParams.append(m[actoinParamName1].as_long())
                        else:
                            break
                      ...
                      workMap.append(
                        {
                            ...
                            "notWorkSet":[],
                            "Valid":True,
                            "transition_formula":actoin["transition_formula"],
                            "precondition":actoin["precondition"],
                            "paramName":actoinParamName1,
                            "isComplete":False,
                            "isWinning":False,
                            "actoinParams":actoinParams,
                            "winning_formula" : '',
                             "paramNumber":len(actoin['action_parameter'])
                        })
                
                self.winSet=list(set(self.winSet))
                for state in self.winSet:
                    for param in (actoinParams if actoinParams else workMap[0]["actoinParams"]):
                ...
                for actionMap in workMap:
                    ...
                    actionMap["workSet"]=list(set(actionMap["workSet"]))
                    actionMap["notWorkSet"]=list(set(actionMap["notWorkSet"]))
            return workMap
        ...
        if self.Game['var_num'] == 2:
            for actoin in self.Game["actions"]:
                ...
                actoinParamName1=actoin['action_parameter'][0]
                if(len(actoin['action_parameter'])==2):
                    actoinParamName2=actoin['action_parameter'][1]
                if(len(actoin['action_parameter'])==3):
                    actoinParamName3=actoin['action_parameter'][2]
                if(not workMapTemp):
                    while(True):
                        s = Solver()
                        s.add(actoin["precondition"])
                        ...
                        if(len(actoin['action_parameter'])==2):
                            for params in actoinParams:
                                s.add(Not(And(actoinParamName1==params[0],actoinParamName2==params[1])))
                        if(len(actoin['action_parameter'])==3):
                            for params in actoinParams:
                                s.add(Not(And(actoinParamName1==params[0],actoinParamName2==params[1],actoinParamName3==params[2])))
                        if(s.check() == sat):
                            m = s.model()
                            if(len(actoin['action_parameter'])==1):
                                actoinParams.append(m[actoinParamName1].as_long())
                            if(len(actoin['action_parameter'])==2):
                                actoinParams.append([m[actoinParamName1].as_long(),m[actoinParamName2].as_long()])
                            if(len(actoin['action_parameter'])==3):
                                actoinParams.append([m[actoinParamName1].as_long(),m[actoinParamName2].as_long(),m[actoinParamName3].as_long()])
                        else:
                            break
                    if(len(actoin['action_parameter'])==1):
                        ...
                        workMap.append(
                            {
                                ...
                                "transition_formula":actoin["transition_formula"],
                                "precondition":actoin["precondition"],
                                "paramName":actoinParamName1 if len(actoin['action_parameter'])==1 else [actoinParamName1,actoinParamName2] if len(actoin['action_parameter'])==2 else [actoinParamName1,actoinParamName2,actoinParamName3],
                                "isComplete":False,
                                "isWinning":False,
                                "actoinParams":actoinParams,
                                "winning_formula" : '',
        ...
        if self.Game['var_num'] == 3:
                ...
                for actoin in self.Game["actions"]:
                    ...
                    if(not workMapTemp):
                        ...
                        actoinParams.append(X2 - X)
                        actoinParams.append(X1 - X2)
                        actoinParams.append(X2 - X1)
                        for param in actoinParams:
                            workMap.append(
                                {
                                    "actionName":actoin["action_name"],
